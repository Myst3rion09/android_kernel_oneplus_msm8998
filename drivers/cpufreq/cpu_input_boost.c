...
static unsigned int dynamic_stune_boost_ms = CONFIG_DYNAMIC_STUNE_BOOST_MS;
#endif
...
#ifdef CONFIG_DYNAMIC_STUNE_BOOST
        cancel_delayed_work_sync(&b->stune_unboost);
...
{
        struct boost_drv *b =
                container_of(to_delayed_work(work), typeof(*b), stune_unboost);
